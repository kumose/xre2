...
#include <xre2/util/malloc_counter.h>
#include <xre2/testing/pcre.h>
...
namespace xre2 {
    void Test() {
        ...
        KCHECK(prog);
        KCHECK(prog->IsOnePass());
        KCHECK(prog->CanBitState());
        const char *text = "650-253-0001";
        std::string_view sp[4];
        KCHECK(prog->SearchOnePass(text, text, Prog::kAnchored, Prog::kFullMatch,
            sp, 4));
        KCHECK_EQ(sp[0], "650-253-0001");
        KCHECK_EQ(sp[1], "650");
        KCHECK_EQ(sp[2], "253");
        KCHECK_EQ(sp[3], "0001");
        delete prog;
        re->Decref();
        KLOG(INFO) << "test passed\n";
    }
    ...
    void MemoryUsage() {
        ...
        const char *text = "650-253-0001";
        {
            ...
            KCHECK(re);
            // Can't pass mc.HeapGrowth() and mc.PeakHeapGrowth() to KLOG(INFO)
            // directly because KLOG(INFO) might do a big allocation before they
            // get evaluated.
            turbo::FPrintF(stderr, "Regexp: %7d bytes (peak=%d)\n",
            ...
            KCHECK(prog->IsOnePass());
            KCHECK(prog->CanBitState());
            turbo::FPrintF(stderr, "Prog:   %7d bytes (peak=%d)\n",
                           mc.HeapGrowth(), mc.PeakHeapGrowth());
            mc.Reset();

            std::string_view sp[4];
            KCHECK(prog->SearchOnePass(text, text, Prog::kAnchored,
                Prog::kFullMatch, sp, 4));
            turbo::FPrintF(stderr, "Search: %7d bytes (peak=%d)\n",
                           mc.HeapGrowth(), mc.PeakHeapGrowth());
            delete prog;
            re->Decref();
        }

        {
            ...
            turbo::FPrintF(stderr, "RE:     %7d bytes (peak=%d)\n",
                           mc.HeapGrowth(), mc.PeakHeapGrowth());
...

        {
            ...
            turbo::FPrintF(stderr, "PCRE*:  %7d bytes (peak=%d)\n",
                           mc.HeapGrowth(), mc.PeakHeapGrowth());
            PCRE::FullMatch(text, *re);
            turbo::FPrintF(stderr, "PCRE*:  %7d bytes (peak=%d)\n",
                           mc.HeapGrowth(), mc.PeakHeapGrowth());
...

        {
            ...
            RE2 re(regexp);
            turbo::FPrintF(stderr, "RE2:    %7d bytes (peak=%d)\n",
                           mc.HeapGrowth(), mc.PeakHeapGrowth());
        ...
        turbo::FPrintF(stderr, "sizeof: PCRE=%d RE2=%d Prog=%d Inst=%d\n",
    ...
    typedef void SearchImpl(benchmark::State &state, const char *regexp,
                            std::string_view text, Prog::Anchor anchor,
                            bool expect_match);
...
#define PARENS     "([ -~])*(A)(B)(C)(D)(E)(F)(G)(H)(I)(J)(K)(L)(M)" \
    ...
    void FindAndConsume(benchmark::State &state) {
        ...
        for (auto _: state) {
            std::string_view t = s;
            std::string_view u;
            KCHECK(RE2::FindAndConsume(&t, re, &u));
            KCHECK_EQ(u, "Hello World");
        }
        state.SetBytesProcessed(state.iterations() * state.range(0));
    }
    ...
    void SearchDigits(benchmark::State &state, SearchImpl *search) {
        std::string_view s("650-253-0001");
...
TURBO_FLAG(std::string, compile_regexp, "(.*)-(\\d+)-of-(\\d+)",
           "regexp for compile benchmarks");

namespace xre2 {
    void BM_PCRE_Compile(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), CompilePCRE);
    }

    void BM_Regexp_Parse(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), ParseRegexp);
    }

    void BM_Regexp_Simplify(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), SimplifyRegexp);
    }

    void BM_CompileToProg(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), CompileToProg);
    }

    void BM_CompileByteMap(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), CompileByteMap);
    }

    void BM_Regexp_Compile(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), CompileRegexp);
    }

    void BM_Regexp_SimplifyCompile(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), SimplifyCompileRegexp);
    }

    void BM_Regexp_NullWalk(benchmark::State &state) {
        RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), NullWalkRegexp);
    }

    void BM_RE2_Compile(benchmark::State &state) { RunBuild(state, turbo::get_flag(FLAGS_compile_regexp), CompileRE2); }
    ...
    */
    ...
    void SearchDFA(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            KCHECK(prog);
            bool failed = false;
            KCHECK_EQ(prog->SearchDFA(text, std::string_view(), anchor,
    ...
    void SearchNFA(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            Prog *prog = re->CompileToProg(0);
            KCHECK(prog);
            KCHECK_EQ(prog->SearchNFA(text, std::string_view(), anchor,
    ...
    void SearchPCRE(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            if (anchor == Prog::kAnchored) {
                KCHECK_EQ(PCRE::FullMatch(text, re), expect_match);
            } else {
                KCHECK_EQ(PCRE::PartialMatch(text, re), expect_match);
            }
        }
    }
    ...
    void SearchRE2(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            KCHECK_EQ(re.error(), "");
            if (anchor == Prog::kAnchored) {
                KCHECK_EQ(RE2::FullMatch(text, re), expect_match);
            } else {
                KCHECK_EQ(RE2::PartialMatch(text, re), expect_match);
            }
        }
    }
    ...
    Prog *GetCachedProg(const char *regexp) {
        ...
        static auto &cache = *new turbo::flat_hash_map<std::string, Prog *>;
    ...
    PCRE *GetCachedPCRE(const char *regexp) {
        static auto &mutex = *new std::mutex;
        std::unique_lock lock(mutex);
        static auto &cache = *new turbo::flat_hash_map<std::string, PCRE *>;
    ...
    RE2 *GetCachedRE2(const char *regexp) {
        static auto &mutex = *new std::mutex;
        std::unique_lock lock(mutex);
        static auto &cache = *new turbo::flat_hash_map<std::string, RE2 *>;
        RE2 *re = cache[regexp];
        if (re == NULL) {
            re = new RE2(regexp);
            KCHECK_EQ(re->error(), "");
    ...
    void Parse3NFA(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            Prog *prog = re->CompileToProg(0);
            KCHECK(prog);
            std::string_view sp[4]; // 4 because sp[0] is whole match.
            KCHECK(prog->SearchNFA(text, std::string_view(), Prog::kAnchored,
                Prog::kFullMatch, sp, 4));
            delete prog;
            re->Decref();
        }
    ...
    void Parse3BitState(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            KCHECK(prog->CanBitState());
            std::string_view sp[4]; // 4 because sp[0] is whole match.
            KCHECK(prog->SearchBitState(text, text, Prog::kAnchored,
                Prog::kFullMatch, sp, 4));
            delete prog;
            re->Decref();
        }
    ...
    void Parse3Backtrack(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            KCHECK(prog);
            std::string_view sp[4]; // 4 because sp[0] is whole match.
            KCHECK(prog->UnsafeSearchBacktrack(text, text, Prog::kAnchored,
                Prog::kFullMatch, sp, 4));
            delete prog;
            re->Decref();
        }
    ...
    void Parse3PCRE(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            PCRE re(regexp, PCRE::UTF8);
            KCHECK_EQ(re.error(), "");
            std::string_view sp1, sp2, sp3;
            KCHECK(PCRE::FullMatch(text, re, &sp1, &sp2, &sp3));
        }
    }
    ...
    void Parse3RE2(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            KCHECK_EQ(re.error(), "");
            std::string_view sp1, sp2, sp3;
            KCHECK(RE2::FullMatch(text, re, &sp1, &sp2, &sp3));
        }
    }
    ...
    void Parse1NFA(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            Prog *prog = re->CompileToProg(0);
            KCHECK(prog);
            std::string_view sp[2]; // 2 because sp[0] is whole match.
            KCHECK(prog->SearchNFA(text, std::string_view(), Prog::kAnchored,
    ...
    void Parse1PCRE(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            KCHECK_EQ(re.error(), "");
            std::string_view sp1;
            KCHECK(PCRE::FullMatch(text, re, &sp1));
        }
    }
    ...
    void Parse1RE2(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            ...
            KCHECK_EQ(re.error(), "");
            std::string_view sp1;
            KCHECK(RE2::FullMatch(text, re, &sp1));
        }
    }
    ...
    void SearchParse2CachedPCRE(benchmark::State &state, const char *regexp,
        ...
        PCRE &re = *GetCachedPCRE(regexp);
        for (auto _: state) {
            std::string_view sp1, sp2;
            KCHECK(PCRE::PartialMatch(text, re, &sp1, &sp2));
        }
    }
    ...
    void SearchParse2CachedRE2(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            std::string_view sp1, sp2;
            KCHECK(RE2::PartialMatch(text, re, &sp1, &sp2));
        }
    }
    ...
    void SearchParse1CachedPCRE(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            std::string_view sp1;
            KCHECK(PCRE::PartialMatch(text, re, &sp1));
        }
    }
    ...
    void SearchParse1CachedRE2(benchmark::State &state, const char *regexp,
        ...
        for (auto _: state) {
            std::string_view sp1;
            KCHECK(RE2::PartialMatch(text, re, &sp1));
        }
    }
    ...
    void PossibleMatchRangeCommon(benchmark::State &state, const char *regexp) {
        ...
        for (auto _: state) {
            KCHECK(re.PossibleMatchRange(&min, &max, kMaxLen));
        }
    }
