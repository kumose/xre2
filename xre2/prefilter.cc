...
namespace xre2 {
    ...
    Prefilter::Info *Prefilter::Info::CClass(CharClass *cc,
                                             bool latin1) {
        if (ExtraDebug) {
            KLOG(ERROR) << "CharClassInfo:";
            for (CCIter i = cc->begin(); i != cc->end(); ++i)
                KLOG(ERROR) << "  " << i->lo << "-" << i->hi;
        }
    ...
    Prefilter::Info *Prefilter::BuildInfo(Regexp *re) {
        if (ExtraDebug)
            KLOG(ERROR) << "BuildPrefilter::Info: " << re->ToString();
    ...
    Prefilter::Info *Prefilter::Info::Walker::PostVisit(
        ...
        switch (re->op()) {
            ...
            case kRegexpRepeat:
                info = EmptyString();
                KLOG(DFATAL) << "Bad regexp op " << re->op();
                break;
        ...
        if (ExtraDebug)
            KLOG(ERROR) << "BuildInfo " << re->ToString()
                    << ": " << (info ? info->ToString() : "");
    ...
    std::string Prefilter::DebugString() const {
        switch (op_) {
            default:
                KLOG(DFATAL) << "Bad op in Prefilter::DebugString: " << op_;
                return turbo::str_format("op%d", op_);
            case NONE:
                return "*no-matches*";
            case ATOM:
                return atom_;
