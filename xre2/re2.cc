...
#include <xre2/sparse_array.h>
#include <xre2/util/strutil.h>
#include <xre2/util/utf.h>
...
namespace xre2 {
    ...
    static RE2::ErrorCode RegexpErrorToRE2(xre2::RegexpStatusCode code) {
        switch (code) {
            case xre2::kRegexpSuccess:
                return RE2::NoError;
            case xre2::kRegexpInternalError:
                return RE2::ErrorInternal;
            case xre2::kRegexpBadEscape:
                return RE2::ErrorBadEscape;
            case xre2::kRegexpBadCharClass:
                return RE2::ErrorBadCharClass;
            case xre2::kRegexpBadCharRange:
                return RE2::ErrorBadCharRange;
            case xre2::kRegexpMissingBracket:
                return RE2::ErrorMissingBracket;
            case xre2::kRegexpMissingParen:
                return RE2::ErrorMissingParen;
            case xre2::kRegexpUnexpectedParen:
                return RE2::ErrorUnexpectedParen;
            case xre2::kRegexpTrailingBackslash:
                return RE2::ErrorTrailingBackslash;
            case xre2::kRegexpRepeatArgument:
                return RE2::ErrorRepeatArgument;
            case xre2::kRegexpRepeatSize:
                return RE2::ErrorRepeatSize;
            case xre2::kRegexpRepeatOp:
                return RE2::ErrorRepeatOp;
            case xre2::kRegexpBadPerlOp:
                return RE2::ErrorBadPerlOp;
            case xre2::kRegexpBadUTF8:
                return RE2::ErrorBadUTF8;
            case xre2::kRegexpBadNamedCapture:
    ...
    RE2::RE2(const char *pattern) {
        // If std::string_view becomes an alias for std::string_view,
    ...
    int RE2::Options::ParseFlags() const {
        ...
        switch (encoding()) {
            default:
                if (log_errors())
                    KLOG(ERROR) << "Unknown encoding " << encoding();
                break;
    ...
    void RE2::Init(std::string_view pattern, const Options &options) {
        ...
        if (entire_regexp_ == NULL) {
            if (options_.log_errors()) {
                KLOG(ERROR) << "Error parsing '" << trunc(*pattern_) << "': "
                      << status.Text();
            }
        ...
        if (prog_ == NULL) {
            if (options_.log_errors())
                KLOG(ERROR) << "Error compiling '" << trunc(*pattern_) << "'";
    ...
    xre2::Prog *RE2::ReverseProg() const {
        std::call_once(rprog_once_, [](const RE2 *re) {
            re->rprog_ =
                    re->suffix_regexp_->CompileToReverseProg(re->options_.max_mem() / 3);
            if (re->rprog_ == NULL) {
                if (re->options_.log_errors())
                    KLOG(ERROR) << "Error reverse compiling '" << trunc(*re->pattern_)
                        << "'";
    ...
    bool RE2::Replace(std::string *str,
        ...
        if (nvec > 1 + re.NumberOfCapturingGroups())
            return false;
        if (nvec > static_cast<int>(TURBO_ARRAYSIZE(vec)))
            return false;
        ...
        DKCHECK_GE(vec[0].data(), str->data());
        DKCHECK_LE(vec[0].data() + vec[0].size(), str->data() + str->size());
    ...
    bool RE2::Match(std::string_view text,
        ...
        if (!ok()) {
            if (options_.log_errors())
                KLOG(ERROR) << "Invalid RE2: " << *error_;
            return false;
        }

        if (startpos > endpos || endpos > text.size()) {
            if (options_.log_errors())
                KLOG(ERROR) << "RE2: invalid startpos, endpos pair. ["
                      << "startpos: " << startpos << ", "
                      << "endpos: " << endpos << ", "
                      << "text size: " << text.size() << "]";
            return false;
        ...
        switch (re_anchor) {
            default:
                KLOG(DFATAL) << "Unexpected re_anchor value: " << re_anchor;
                return false;
            ...
            case UNANCHORED: {
                ...
                if (!prog->SearchDFA(match, text, Prog::kAnchored,
                    ...
                    if (options_.log_errors())
                        KLOG(ERROR) << "SearchDFA inconsistency";
                    return false;
                }
                break;
            }
            ...
            if (can_one_pass && anchor != Prog::kUnanchored) {
                if (!prog_->SearchOnePass(subtext1, text, anchor, kind, submatch, ncap)) {
                    if (!skipped_test && options_.log_errors())
                        KLOG(ERROR) << "SearchOnePass inconsistency";
                    return false;
                }
                ...
                if (!prog_->SearchBitState(subtext1, text, anchor,
                                           kind, submatch, ncap)) {
                    if (!skipped_test && options_.log_errors())
                        KLOG(ERROR) << "SearchBitState inconsistency";
                    return false;
                }
            } else {
                if (!prog_->SearchNFA(subtext1, text, anchor, kind, submatch, ncap)) {
                    if (!skipped_test && options_.log_errors())
                        KLOG(ERROR) << "SearchNFA inconsistency";
                    return false;
                }
            }
        }
    ...
    bool RE2::DoMatch(std::string_view text,
        ...
        turbo::FixedArray<std::string_view, kVecSize> vec_storage(nvec);
        std::string_view *vec = vec_storage.data();
    ...
    bool RE2::Rewrite(std::string *out,
        ...
        for (const char *s = rewrite.data(), *end = s + rewrite.size();
            ...
            if (turbo::ascii_isdigit(c)) {
                ...
                if (n >= veclen) {
                    if (options_.log_errors()) {
                        KLOG(ERROR) << "invalid substitution \\" << n
                          << " from " << veclen << " groups";
                    }
                    return false;
                }
                ...
                if (options_.log_errors())
                    KLOG(ERROR) << "invalid rewrite pattern: " << rewrite;
                return false;
            }
        }
        return true;
