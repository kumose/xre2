...
Regexp::~Regexp() {
  if (nsub_ > 0)
    KLOG(DFATAL) << "Regexp not destroyed.";
...
struct RefStorage {
  std::mutex ref_mutex;
  turbo::flat_hash_map<Regexp*, int> ref_map;
};
...
static inline turbo::flat_hash_map<Regexp*, int>* ref_map() {
...
void Regexp::Destroy() {
  ...
  while (stack != NULL) {
    ...
    stack = re->down_;
    if (re->ref_ != 0)
      KLOG(DFATAL) << "Bad reference count " << re->ref_;
    if (re->nsub_ > 0) {
      Regexp** subs = re->sub();
...
void Regexp::AddRuneToString(Rune r) {
  DKCHECK(op_ == kRegexpLiteralString);
  if (nrunes_ == 0) {
    // start with 8
    runes_ = new Rune[8];
...
static bool TopEqual(Regexp* a, Regexp* b) {
  ...
  KLOG(DFATAL) << "Unexpected op in Regexp::Equal: " << a->op();
  return 0;
}
...
std::string RegexpStatus::CodeText(enum RegexpStatusCode code) {
  if (code < 0 || code >= TURBO_ARRAYSIZE(kErrorStrings))
...
class NumCapturesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  virtual Ignored ShortVisit(Regexp* re, Ignored ignored) {
    ...
    KLOG(DFATAL) << "NumCapturesWalker::ShortVisit called";
#endif
    return ignored;
  }
...
class NamedCapturesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  virtual Ignored ShortVisit(Regexp* re, Ignored ignored) {
    // Should never be called: we use Walk(), not WalkExponential().
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
    KLOG(DFATAL) << "NamedCapturesWalker::ShortVisit called";
#endif
    return ignored;
  }
...
class CaptureNamesWalker : public Regexp::Walker<Ignored> {
 public:
  ...
  virtual Ignored ShortVisit(Regexp* re, Ignored ignored) {
    // Should never be called: we use Walk(), not WalkExponential().
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
    KLOG(DFATAL) << "CaptureNamesWalker::ShortVisit called";
#endif
    return ignored;
  }
...
CharClass* CharClassBuilder::GetCharClass() {
  ...
  for (iterator it = begin(); it != end(); ++it)
    cc->ranges_[n++] = *it;
  cc->nranges_ = n;
  DKCHECK_LE(n, static_cast<int>(ranges_.size()));
  cc->nrunes_ = nrunes_;
