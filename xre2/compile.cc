...
class Compiler : public Regexp::Walker<Frag> {
 ...
 private:
  ...
  turbo::flat_hash_map<uint64_t, int> rune_cache_;
  Frag rune_range_;
...
int Compiler::CachedRuneByteSuffix(uint8_t lo, uint8_t hi, bool foldcase,
  ...
  turbo::flat_hash_map<uint64_t, int>::const_iterator it = rune_cache_.find(key);
...
int Compiler::AddSuffixRecursive(int root, int id) {
  DKCHECK(inst_[root].opcode() == kInstAlt ||
...
Frag Compiler::FindByteRange(int root, int id) {
  ...
  KLOG(DFATAL) << "should never happen";
  return NoMatch();
}
...
Frag Compiler::Copy(Frag arg) {
  ...
  failed_ = true;
  KLOG(DFATAL) << "Compiler::Copy called!";
  return NoMatch();
}
...
Frag Compiler::PostVisit(Regexp* re, Frag, Frag, Frag* child_frags,
  ...
  switch (re->op()) {
    ...
    case kRegexpCharClass: {
      ...
      if (cc->empty()) {
        // This can't happen.
        failed_ = true;
        KLOG(DFATAL) << "No ranges in char class";
        return NoMatch();
      }
  ...
  failed_ = true;
  KLOG(DFATAL) << "Missing case in Compiler: " << re->op();
  return NoMatch();
}
