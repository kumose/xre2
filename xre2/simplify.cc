...
bool Regexp::ComputeSimple() {
  ...
  KLOG(DFATAL) << "Case not handled in ComputeSimple: " << op_;
  return false;
}
...
Regexp* CoalesceWalker::ShortVisit(Regexp* re, Regexp* parent_arg) {
  ...
  KLOG(DFATAL) << "CoalesceWalker::ShortVisit called";
#endif
  return re->Incref();
}
...
void CoalesceWalker::DoCoalesce(Regexp** r1ptr, Regexp** r2ptr) {
  ...
  switch (r1->op()) {
    ...
    default:
      nre->Decref();
      KLOG(DFATAL) << "DoCoalesce failed: r1->op() is " << r1->op();
      return;
  }
  ...
  switch (r2->op()) {
    ...
    default:
      nre->Decref();
      KLOG(DFATAL) << "DoCoalesce failed: r2->op() is " << r2->op();
      return;
  }
...
Regexp* SimplifyWalker::ShortVisit(Regexp* re, Regexp* parent_arg) {
  // Should never be called: we use Walk(), not WalkExponential().
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
  KLOG(DFATAL) << "SimplifyWalker::ShortVisit called";
#endif
  return re->Incref();
}
...
Regexp* SimplifyWalker::PostVisit(Regexp* re,
  ...
  KLOG(ERROR) << "Simplify case not handled: " << re->op();
  return re->Incref();
}
...
Regexp* SimplifyWalker::SimplifyRepeat(Regexp* re, int min, int max,
  ...
  if (nre == NULL) {
    ...
    KLOG(DFATAL) << "Malformed repeat of " << re->ToString()
                     << " min " << min << " max " << max;
