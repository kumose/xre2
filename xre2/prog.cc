...
namespace xre2 {
    ...
    void Prog::Inst::InitAlt(uint32_t out, uint32_t out1) {
        DKCHECK_EQ(out_opcode_, uint32_t{0});
    ...
    std::string Prog::Inst::Dump() {
        switch (opcode()) {
            default:
                return turbo::str_format("opcode %d", static_cast<int>(opcode()));

            case kInstAlt:
                return turbo::str_format("alt -> %d | %d", out(), out1_);

            case kInstAltMatch:
                return turbo::str_format("altmatch -> %d | %d", out(), out1_);

            case kInstByteRange:
                return turbo::str_format("byte%s [%02x-%02x] %d -> %d",
                                         foldcase() ? "/i" : "",
                                         lo_, hi_, hint(), out());

            case kInstCapture:
                return turbo::str_format("capture %d -> %d", cap_, out());

            case kInstEmptyWidth:
                return turbo::str_format("emptywidth %#x -> %d",
                                         static_cast<int>(empty_), out());

            case kInstMatch:
                return turbo::str_format("match! %d", match_id());

            case kInstNop:
                return turbo::str_format("nop -> %d", out());

            case kInstFail:
                return turbo::str_format("fail");
        }
    }
    ...
    std::string Prog::DumpByteMap() {
        ...
        for (int c = 0; c < 256; c++) {
            ...
            while (c < 256 - 1 && bytemap_[c + 1] == b)
                c++;
            int hi = c;
            map += turbo::str_format("[%02x-%02x] -> %d\n", lo, hi, b);
        }
        return map;
    }
    ...
    static bool IsMatch(Prog *prog, Prog::Inst *ip) {
        for (;;) {
            switch (ip->opcode()) {
                default:
                    KLOG(DFATAL) << "Unexpected opcode in IsMatch: " << ip->opcode();
                    return false;
    ...
    void Prog::ComputeByteMap() {
        ...
        if ((0)) {
            // For debugging, use trivial bytemap.
            KLOG(ERROR) << "Using trivial bytemap.";
            for (int i = 0; i < 256; i++)
    ...
    const void *Prog::PrefixAccel_FrontAndBack(const void *data, size_t size) {
        DKCHECK_GE(prefix_size_, size_t{2});
        if (size < prefix_size_)
            return NULL;
        ...
        if (size >= sizeof(__m256i)) {
            ...
            do {
                ...
                if (fb_testz == 0) {
                    // ZF: 1 means zero, 0 means non-zero.
        ...
        for (const char *p = p0;; p++) {
            DKCHECK_GE(size, static_cast<size_t>(p-p0));
