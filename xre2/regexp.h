...
class Regexp {
 public:
  ...
  Rune rune() {
    DKCHECK_EQ(op_, kRegexpLiteral);
    return rune_;
  }
  CharClass* cc() {
    DKCHECK_EQ(op_, kRegexpCharClass);
    return cc_;
  }
  int cap() {
    DKCHECK_EQ(op_, kRegexpCapture);
    return cap_;
  }
  ...
  Rune* runes() {
    DKCHECK_EQ(op_, kRegexpLiteralString);
    return runes_;
  }
  int nrunes() {
    DKCHECK_EQ(op_, kRegexpLiteralString);
  ...
  int match_id() {
    DKCHECK_EQ(op_, kRegexpHaveMatch);
    return match_id_;
  }
 ...
 private:
  ...
  void AllocSub(int n) {
    DKCHECK(n >= 0 && static_cast<uint16_t>(n) == n);
    if (n > 1)
