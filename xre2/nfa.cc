...
class NFA {
 public:
  ...
  bool Search(std::string_view text, std::string_view context, bool anchored,
              bool longest, std::string_view* submatch, int nsubmatch);
...
void NFA::AddToThreadq(Threadq* q, int id0, int c, std::string_view context,
  ...
  Loop:
    ...
    switch (ip->opcode()) {
    default:
      KLOG(DFATAL) << "unhandled " << ip->opcode() << " in AddToThreadq";
      break;
  ...
  for (Threadq::iterator i = runq->begin(); i != runq->end(); ++i) {
    ...
    switch (ip->opcode()) {
      default:
        ...
        KLOG(DFATAL) << "Unhandled " << ip->opcode() << " in step";
        break;
...
std::string NFA::FormatCapture(const char** capture) {
  ...
  for (int i = 0; i < ncapture_; i+=2) {
    if (capture[i] == NULL)
      ...
      s += turbo::str_format("(%d,?)",
                           capture[i] - btext_);
    else
      s += turbo::str_format("(%d,%d)",
                           capture[i] - btext_,
                           capture[i+1] - btext_);
  }
  return s;
...
bool NFA::Search(std::string_view text, std::string_view context,
                 bool anchored, bool longest, std::string_view* submatch,
                 int nsubmatch) {
  if (start_ == 0)
    return false;
  ...
  if (nsubmatch < 0) {
    KLOG(DFATAL) << "Bad args: nsubmatch=" << nsubmatch;
    return false;
  }
  ...
  if (ExtraDebug)
    turbo::FPrintF(stderr, "NFA::Search %s (context: %s) anchored=%d longest=%d\n",
  ...
  for (const char* p = text.data();; p++) {
    if (ExtraDebug) {
      ...
      turbo::FPrintF(stderr, "%c:", c);
      for (Threadq::iterator i = runq->begin(); i != runq->end(); ++i) {
        Thread* t = i->value();
        if (t == NULL)
          continue;
        turbo::FPrintF(stderr, " %d%s", i->index(), FormatCapture(t->capture));
      }
    ...
    if (id != 0) {
      ...
      for (;;) {
        ...
        switch (ip->opcode()) {
          default:
            KLOG(DFATAL) << "Unexpected opcode in short circuit: "
                             << ip->opcode();
            break;
  ...
  if (matched_) {
    ...
    if (ExtraDebug)
      turbo::FPrintF(stderr, "match (%d,%d)\n",
                    match_[0] - btext_,
                    match_[1] - btext_);
    return true;
  }
...
bool Prog::SearchNFA(std::string_view text, std::string_view context,
                     Anchor anchor, MatchKind kind, std::string_view* match,
                     int nmatch) {
  if (ExtraDebug)
    Dump();
...
void Prog::Fanout(SparseArray<int>* fanout) {
  DKCHECK_EQ(fanout->max_size(), size());
  ...
  for (SparseArray<int>::iterator i = fanout->begin(); i != fanout->end(); ++i) {
    ...
    for (SparseSet::iterator j = reachable.begin(); j != reachable.end(); ++j) {
      ...
      switch (ip->opcode()) {
        default:
          KLOG(DFATAL) << "unhandled " << ip->opcode()
                           << " in Prog::Fanout()";
          break;
