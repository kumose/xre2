...
int RE2::Set::Add(std::string_view pattern, std::string* error) {
  if (compiled_) {
    KLOG(DFATAL) << "RE2::Set::Add() called after compiling";
    return -1;
  }
  ...
  Regexp::ParseFlags pf = static_cast<Regexp::ParseFlags>(
    options_.ParseFlags());
  RegexpStatus status;
  xre2::Regexp* re = Regexp::Parse(pattern, pf, &status);
  if (re == NULL) {
    if (error != NULL)
      *error = status.Text();
    if (options_.log_errors())
      KLOG(ERROR) << "Error parsing '" << pattern << "': " << status.Text();
    return -1;
  }

  // Concatenate with match index and push on vector.
  int n = static_cast<int>(elem_.size());
  xre2::Regexp* m = xre2::Regexp::HaveMatch(n, pf);
  if (re->op() == kRegexpConcat) {
    int nsub = re->nsub();
    PODArray<xre2::Regexp*> sub(nsub + 1);
    for (int i = 0; i < nsub; i++)
      sub[i] = re->sub()[i]->Incref();
    sub[nsub] = m;
    re->Decref();
    re = xre2::Regexp::Concat(sub.data(), nsub + 1, pf);
  } else {
    xre2::Regexp* sub[2];
    sub[0] = re;
    sub[1] = m;
    re = xre2::Regexp::Concat(sub, 2, pf);
  }
...
bool RE2::Set::Compile() {
  if (compiled_) {
    KLOG(DFATAL) << "RE2::Set::Compile() called more than once";
    return false;
  }
  compiled_ = true;
  ...
  PODArray<xre2::Regexp*> sub(size_);
  ...
  Regexp::ParseFlags pf = static_cast<Regexp::ParseFlags>(
    options_.ParseFlags());
  xre2::Regexp* re = xre2::Regexp::Alternate(sub.data(), size_, pf);
...
bool RE2::Set::Match(std::string_view text, std::vector<int>* v,
  ...
  if (!compiled_) {
    ...
    KLOG(DFATAL) << "RE2::Set::Match() called before compiling";
    return false;
  }
  ...
  if (v != NULL) {
    if (matches->empty()) {
      ...
      KLOG(DFATAL) << "RE2::Set::Match() matched, but no matches returned";
      return false;
    }
