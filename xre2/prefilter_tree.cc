...
namespace xre2 {
    ...
    void PrefilterTree::Add(Prefilter *prefilter) {
        if (compiled_) {
            KLOG(DFATAL) << "Add called after Compile.";
            return;
        }
    ...
    void PrefilterTree::Compile(std::vector<std::string> *atom_vec) {
        if (compiled_) {
            KLOG(DFATAL) << "Compile called already.";
            return;
        }
    ...
    bool PrefilterTree::KeepNode(Prefilter *node) const {
        ...
        switch (node->op()) {
            default:
                KLOG(DFATAL) << "Unexpected op in KeepNode: " << node->op();
                return false;
    ...
    void PrefilterTree::AssignUniqueIds(NodeSet *nodes,
        ...
        for (int i = static_cast<int>(v.size()) - 1; i >= 0; i--) {
            ...
            switch (prefilter->op()) {
                default:
                    KLOG(DFATAL) << "Unexpected op: " << prefilter->op();
                    return;
    ...
    void PrefilterTree::RegexpsGivenStrings(
        ...
        if (!compiled_) {
            ...
            KLOG(ERROR) << "RegexpsGivenStrings called before Compile.";
            for (size_t i = 0; i < prefilter_vec_.size(); i++)
    ...
    void PrefilterTree::PrintPrefilter(int regexpid) {
        KLOG(ERROR) << DebugNodeString(prefilter_vec_[regexpid]);
    }

    void PrefilterTree::PrintDebugInfo(NodeSet *nodes) {
        KLOG(ERROR) << "#Unique Atoms: " << atom_index_to_id_.size();
        KLOG(ERROR) << "#Unique Nodes: " << entries_.size();

        for (size_t i = 0; i < entries_.size(); i++) {
            const std::vector<int> &parents = entries_[i].parents;
            const std::vector<int> &regexps = entries_[i].regexps;
            KLOG(ERROR) << "EntryId: " << i
    ...
    std::string PrefilterTree::DebugNodeString(Prefilter *node) const {
        ...
        if (node->op() == Prefilter::ATOM) {
            ...
            for (size_t i = 0; i < node->subs()->size(); i++) {
                if (i > 0)
                    node_string += ',';
                node_string += turbo::str_format("%d", (*node->subs())[i]->unique_id());
                node_string += ":";
