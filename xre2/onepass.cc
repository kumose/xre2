...
namespace xre2 {
    ...
    bool Prog::SearchOnePass(std::string_view text, std::string_view context,
                             Anchor anchor, MatchKind kind,
                             std::string_view *match, int nmatch) {
        if (anchor != kAnchored && kind != kFullMatch) {
            KLOG(DFATAL) << "Cannot use SearchOnePass for unanchored matches.";
            return false;
        }
    ...
    bool Prog::IsOnePass() {
        ...
        turbo::FixedArray<InstCond, 64> stack_storage(stacksize);
        InstCond *stack = stack_storage.data();

        int size = this->size();
        turbo::FixedArray<int, 128> nodebyid_storage(size, -1); // indexed by ip
        ...
        turbo::InlinedVector<uint8_t, 2048> nodes;
        ...
        if (ExtraDebug) {
            // For debugging, dump one-pass NFA to KLOG(ERROR).
            KLOG(ERROR) << "bytemap:\n" << DumpByteMap();
            KLOG(ERROR) << "prog:\n" << Dump();
            ...
            for (Instq::iterator it = tovisit.begin(); it != tovisit.end(); ++it) {
                ...
                if (nodeindex == -1)
                    continue;
                OneState *node = IndexToNode(nodes.data(), statesize, nodeindex);
                dump += turbo::str_format("node %d id=%d: matchcond=%#x\n",
                                          nodeindex, id, node->matchcond);
                for (int i = 0; i < bytemap_range_; i++) {
                    if ((node->action[i] & kImpossible) == kImpossible)
                        continue;
                    dump += turbo::str_format("  %d cond %#x -> %d id=%d\n",
