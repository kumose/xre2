...
#include <xre2/unicode_casefold.h>
#include <xre2/unicode_groups.h>
...
namespace xre2 {
    ...
    static void AddFoldedRange(CharClassBuilder *cc, Rune lo, Rune hi, int depth) {
        ...
        if (depth > 10) {
            KLOG(DFATAL) << "AddFoldedRange recurses too much.";
            return;
        }
    ...
    int RepetitionWalker::ShortVisit(Regexp *re, int parent_arg) {
        ...
        KLOG(DFATAL) << "RepetitionWalker::ShortVisit called";
#endif
        return 0;
    }
                ...
                for (int i = 0; i < nsub;) {
                    ...
                    switch (round) {
                        ...
                        case 3:
                            ...
                            break;
                        default:
                            KLOG(DFATAL) << "unknown round: " << round;
                            break;
                    }
            ...
            if (i == start) {
                ...
                for (int j = start; j < i; j++) {
                    ...
                    if (re->op() == kRegexpCharClass) {
                        ...
                        KLOG(DFATAL) << "RE2: unexpected op: " << re->op() << " "
                           << re->ToString();
                    }
                    re->Decref();
                }
    ...
    static bool ParseInteger(std::string_view *s, int *np) {
        if (s->empty() || !turbo::ascii_isdigit((*s)[0] & 0xFF))
            return false;
        // Disallow leading zeros.
        if (s->size() >= 2 && (*s)[0] == '0' && turbo::ascii_isdigit((*s)[1] & 0xFF))
            return false;
        int n = 0;
        int c;
        while (!s->empty() && turbo::ascii_isdigit(c = (*s)[0] & 0xFF)) {
            // Avoid overflow.
            if (n >= 100000000)
                return false;
            n = n * 10 + c - '0';
    ...
    static bool ParseEscape(std::string_view *s, Rune *rp,
        ...
        switch (c) {
            default:
                if (c < Runeself && !turbo::ascii_isalnum(c)) {
    ...
    bool Regexp::ParseState::ParsePerlFlags(std::string_view *s) {
        ...
        if (!(flags_ & PerlX) || t.size() < 2 || t[0] != '(' || t[1] != '?') {
            ...
            KLOG(DFATAL) << "Bad call to ParseState::ParsePerlFlags";
            return false;
        }
