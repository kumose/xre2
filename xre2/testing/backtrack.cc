...
bool Backtracker::Search(std::string_view text, std::string_view context,
  ...
  KCHECK_LT(2*nsubmatch_, static_cast<int>(TURBO_ARRAYSIZE(cap_)));
...
bool Backtracker::Try(int id, const char* p) {
  ...
  switch (ip->opcode()) {
    default:
      KLOG(FATAL) << "Unexpected opcode: " << ip->opcode();
    ...
    case kInstCapture:
      if (0 <= ip->cap() &&
          ip->cap() < static_cast<int>(TURBO_ARRAYSIZE(cap_))) {
...
// Runs a backtracking search.
bool Prog::UnsafeSearchBacktrack(std::string_view text,
                                 std::string_view context, Anchor anchor,
                                 MatchKind kind, std::string_view* match,
                                 int nmatch) {
