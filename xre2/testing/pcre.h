...
// compilation as PCRE in namespace xre2.
...
//    KCHECK(PCRE::FullMatch("hello", "h.*o"));
//
// Example: unsuccessful match (requires full match):
//    KCHECK(!PCRE::FullMatch("hello", "e"));
//
// -----------------------------------------------------------------------
...
//    KCHECK(PCRE::FullMatch(utf8_string, re));
//
// -----------------------------------------------------------------------
...
//    int i;
//    std::string s;
//    KCHECK(PCRE::FullMatch("ruby:1234", "(\\w+):(\\d+)", &s, &i));
//
// Example: fails because string cannot be stored in integer
//    KCHECK(!PCRE::FullMatch("ruby", "(.*)", &i));
//
// Example: fails because there aren't enough sub-patterns:
//    KCHECK(!PCRE::FullMatch("ruby:1234", "\\w+:\\d+", &s));
//
// Example: does not try to extract any extra sub-patterns
//    KCHECK(PCRE::FullMatch("ruby:1234", "(\\w+):(\\d+)", &s));
//
// Example: does not try to extract into NULL
//    KCHECK(PCRE::FullMatch("ruby:1234", "(\\w+):(\\d+)", NULL, &i));
//
// Example: integer overflow causes failure
//    KCHECK(!PCRE::FullMatch("ruby:1234567891234", "\\w+:(\\d+)", &i));
//
// -----------------------------------------------------------------------
// PARTIAL MATCHES
//
...
//      KCHECK(PCRE::PartialMatch("hello", "ell"));
//
// Example: find first number in a string
//      int number;
//      KCHECK(PCRE::PartialMatch("x*100 + 20", "(\\d+)", &number));
//      KCHECK_EQ(number, 100);
//
// -----------------------------------------------------------------------
...
//      std::string_view input(contents);  // Wrap a string_view around it
//
//      std::string var;
//      int value;
...
// Example:
//   int a, b, c, d;
//   KCHECK(PCRE::FullMatch("100 40 0100 0x40", "(.*) (.*) (.*) (.*)",
...
class PCRE {
 public:
  ...
  //    std::string_view  (string_view is mutated to point to matched piece)
  ...
  struct FullMatchFunctor {
    bool operator ()(std::string_view text, const PCRE& re,  // 3..16 args
  ...
  struct ConsumeFunctor {
    bool operator ()(std::string_view* input, const PCRE& pattern,  // 3..16 args
                     const Arg& ptr1 = no_more_args,
                     const Arg& ptr2 = no_more_args,
                     const Arg& ptr3 = no_more_args,
                     const Arg& ptr4 = no_more_args,
  ...
  //
  //   std::string s = "yabba dabba doo";
  //   KCHECK(PCRE::GlobalReplace(&s, "b+", "d"));
  //
  ...
  static bool Extract(std::string_view text, const PCRE& pattern,
                      std::string_view rewrite, std::string* out);
  ...
  // operations won't KLOG(ERROR) or fail because of a bad rewrite
  // string.
  ...
  bool CheckRewriteString(std::string_view rewrite, std::string* error) const;
  ...
  bool DoMatch(std::string_view text, Anchor anchor, size_t* consumed,
 ...
 private:
  ...
  int TryMatch(std::string_view text, size_t startpos, Anchor anchor,
               bool empty_ok, int* vec, int vecsize) const;

  // Append the "rewrite" string, with backslash substitutions from "text"
  // and "vec", to string "out".
  bool Rewrite(std::string* out, std::string_view rewrite,
               std::string_view text, int* vec, int veclen) const;

  // internal implementation for DoMatch
  bool DoMatchImpl(std::string_view text, Anchor anchor, size_t* consumed,
...
#define MAKE_PARSER(type, name)            \
  ...
  MAKE_PARSER(std::string_view,  parse_string_view);
