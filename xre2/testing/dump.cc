...
static void DumpRegexpAppending(Regexp* re, std::string* s) {
  if (re->op() < 0 || re->op() >= TURBO_ARRAYSIZE(kOpcodeNames)) {
    *s += turbo::str_format("op%d", re->op());
  } else {
    switch (re->op()) {
      default:
        break;
  ...
  switch (re->op()) {
    ...
    case kRegexpCapture:
      ...
      break;
    case kRegexpRepeat:
      s->append(turbo::str_format("%d,%d ", re->min(), re->max()));
      DumpRegexpAppending(re->sub()[0], s);
      break;
    case kRegexpCharClass: {
      ...
      for (CharClass::iterator it = re->cc()->begin();
        ...
        s->append(sep);
        if (rr.lo == rr.hi)
          s->append(turbo::str_format("%#x", rr.lo));
        else
          s->append(turbo::str_format("%#x-%#x", rr.lo, rr.hi));
        sep = " ";
      }
      break;
    }
