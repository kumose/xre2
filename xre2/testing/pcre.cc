...
#define PCREPORT(level) KLOG(level)
...
TURBO_FLAG(int, regexp_stack_limit, 256 << 10,
          "default PCRE stack limit (bytes)");
TURBO_FLAG(int, regexp_match_limit, 1000000,
...
bool PCRE::FullMatchFunctor::operator()(
    std::string_view text, const PCRE& re, const Arg& a0, const Arg& a1,
...
bool PCRE::ConsumeFunctor::operator()(
    std::string_view* input, const PCRE& pattern, const Arg& a0, const Arg& a1,
    const Arg& a2, const Arg& a3, const Arg& a4, const Arg& a5, const Arg& a6,
    const Arg& a7, const Arg& a8, const Arg& a9, const Arg& a10, const Arg& a11,
    const Arg& a12, const Arg& a13, const Arg& a14, const Arg& a15) const {
  const Arg* args[kMaxArgs];
...
bool PCRE::Extract(std::string_view text, const PCRE& pattern,
...
std::string PCRE::QuoteMeta(std::string_view unquoted) {
  std::string result;
...
int PCRE::TryMatch(std::string_view text, size_t startpos, Anchor anchor,
  ...
  if (match_limit <= 0) {
    match_limit = turbo::get_flag(FLAGS_regexp_match_limit);
  }

  int stack_limit = stack_limit_;
  if (stack_limit <= 0) {
    stack_limit = turbo::get_flag(FLAGS_regexp_stack_limit);
  }
...
bool PCRE::DoMatchImpl(std::string_view text, Anchor anchor, size_t* consumed,
...
bool PCRE::DoMatch(std::string_view text, Anchor anchor, size_t* consumed,
...
bool PCRE::Rewrite(std::string* out, std::string_view rewrite,
...
bool PCRE::CheckRewriteString(std::string_view rewrite,
