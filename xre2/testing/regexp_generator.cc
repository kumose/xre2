...
const std::vector<std::string>& RegexpGenerator::EgrepOps() {
  ...
  static std::vector<std::string> v(ops, ops + TURBO_ARRAYSIZE(ops));
  return v;
}
...
void RegexpGenerator::RunPostfix(const std::vector<std::string>& post) {
  ...
  for (size_t i = 0; i < post.size(); i++) {
    switch (CountArgs(post[i])) {
      default:
        KLOG(FATAL) << "Bad operator: " << post[i];
      case 0:
        regexps.push(post[i]);
        break;
      case 1: {
        auto fmt = turbo::ParsedFormat<'s'>::New(post[i]);
        KCHECK(fmt != nullptr);
        std::string a = regexps.top();
        regexps.pop();
        regexps.push("(?:" + turbo::str_format(*fmt, a) + ")");
        break;
      }
      case 2: {
        auto fmt = turbo::ParsedFormat<'s', 's'>::New(post[i]);
        KCHECK(fmt != nullptr);
        std::string b = regexps.top();
        regexps.pop();
        std::string a = regexps.top();
        regexps.pop();
        regexps.push("(?:" + turbo::str_format(*fmt, a, b) + ")");
        break;
      }
    }
  }
  ...
  if (regexps.size() != 1) {
    ...
    turbo::PrintF("Bad regexp program:\n");
    for (size_t i = 0; i < post.size(); i++) {
      turbo::PrintF("  %s\n", turbo::c_encode(post[i]));
    }
    turbo::PrintF("Stack after running program:\n");
    while (!regexps.empty()) {
      turbo::PrintF("  %s\n", turbo::c_encode(regexps.top()));
      regexps.pop();
    }
    KLOG(FATAL) << "Bad regexp program.";
  }
...
// vector of pieces.
std::vector<std::string> Split(std::string_view sep, std::string_view s) {
  std::vector<std::string> v;
  ...
  for (const char *q = s.data(); q + sep.size() <= s.data() + s.size(); q++) {
    if (std::string_view(q, sep.size()) == sep) {
      v.push_back(std::string(p, q - p));
      p = q + sep.size();
      q = p - 1;  // -1 for ++ in loop
      continue;
