...
class TestInstance {
 public:
  ...
  TestInstance(std::string_view regexp, Prog::MatchKind kind,
  ...
  bool RunCase(std::string_view text, std::string_view context,
               Prog::Anchor anchor);

 private:
  // Runs a single search using the named engine type.
  void RunSearch(Engine type, std::string_view text, std::string_view context,
                 Prog::Anchor anchor, Result* result);

  void LogMatch(const char* prefix, Engine e, std::string_view text,
                std::string_view context, Prog::Anchor anchor);

  std::string_view regexp_str_;    // regexp being tested
...
class Tester {
 public:
  explicit Tester(std::string_view regexp);
  ~Tester();
  ...
  // Runs a single test case: search in text, which is in context,
  // using the given anchoring.
  bool TestCase(std::string_view text, std::string_view context,
                Prog::Anchor anchor);

  // Run TestCase(text, text, anchor) for all anchoring modes.
  bool TestInput(std::string_view text);

  // Run TestCase(text, context, anchor) for all anchoring modes.
  bool TestInputInContext(std::string_view text, std::string_view context);
...
bool TestRegexpOnText(std::string_view regexp, std::string_view text);
