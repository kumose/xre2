...
TURBO_FLAG(bool, dump_prog, false, "dump regexp program");
TURBO_FLAG(bool, log_okay, false, "log successful runs");
TURBO_FLAG(bool, dump_rprog, false, "dump reversed regexp program");

TURBO_FLAG(int, max_regexp_failures, 100,
          "maximum number of regexp test failures (-1 = unlimited)");

TURBO_FLAG(std::string, regexp_engines, "",
...
static const char* EngineName(Engine e) {
  KCHECK_GE(e, 0);
  KCHECK_LT(e, TURBO_ARRAYSIZE(engine_names));
  KCHECK(engine_names[e] != NULL);
...
static uint32_t Engines() {
  ...
  if (turbo::get_flag(FLAGS_regexp_engines).empty()) {
    cached_engines = ~0;
  } else {
    for (Engine i = static_cast<Engine>(0); i < kEngineMax; i++)
      if (turbo::get_flag(FLAGS_regexp_engines).find(EngineName(i)) != std::string::npos)
        cached_engines |= 1<<i;
  }

  if (cached_engines == 0)
    KLOG(INFO) << "Warning: no engines enabled.";
  if (!UsingPCRE)
    cached_engines &= ~(1<<kEnginePCRE);
  for (Engine i = static_cast<Engine>(0); i < kEngineMax; i++) {
    if (cached_engines & (1<<i))
      KLOG(INFO) << EngineName(i) << " enabled";
  }
...
struct TestInstance::Result {
  ...
  std::string_view submatch[kMaxSubmatch];
};
...
static std::string FormatCapture(std::string_view text,
                                 std::string_view s) {
  if (s.data() == NULL)
    return "(?,?)";
  return turbo::str_format("(%d,%d)",
                         BeginPtr(s) - BeginPtr(text),
                         EndPtr(s) - BeginPtr(text));
}

// Returns whether text contains non-ASCII (>= 0x80) bytes.
static bool NonASCII(std::string_view text) {
...
static std::string FormatMode(Regexp::ParseFlags flags) {
  for (size_t i = 0; i < TURBO_ARRAYSIZE(parse_modes); i++)
    if (parse_modes[i].parse_flags == flags)
      return parse_modes[i].desc;
  return turbo::str_format("%#x", static_cast<uint32_t>(flags));
}

// Constructs and saves all the matching engines that
// will be required for the given tests.
TestInstance::TestInstance(std::string_view regexp_str, Prog::MatchKind kind,
  ...
  VKLOG(1) << turbo::c_encode(regexp_str);
  ...
  if (regexp_ == NULL) {
    KLOG(INFO) << "Cannot parse: " << turbo::c_encode(regexp_str_)
  ...
  if (prog_ == NULL) {
    KLOG(INFO) << "Cannot compile: " << turbo::c_encode(regexp_str_);
    error_ = true;
    return;
  }
  if (turbo::get_flag(FLAGS_dump_prog)) {
    KLOG(INFO) << "Prog for "
                   << " regexp "
                   << turbo::c_encode(regexp_str_)
                   << " (" << FormatKind(kind_)
                   << ", " << FormatMode(flags_)
                   << ")\n"
                   << prog_->Dump();
  ...
  if (Engines() & ((1<<kEngineDFA)|(1<<kEngineDFA1))) {
    ...
    if (rprog_ == NULL) {
      KLOG(INFO) << "Cannot reverse compile: "
                     << turbo::c_encode(regexp_str_);
      error_ = true;
      return;
    }
    if (turbo::get_flag(FLAGS_dump_rprog))
      KLOG(INFO) << rprog_->Dump();
  }
  ...
  if (Engines() & ((1<<kEngineRE2)|(1<<kEngineRE2a)|(1<<kEngineRE2b))) {
    ...
    if (!re2_->error().empty()) {
      KLOG(INFO) << "Cannot RE2: " << turbo::c_encode(re);
      error_ = true;
      return;
    }
  }
  ...
  if ((Engines() & (1<<kEnginePCRE)) && regexp_->MimicsPCRE() &&
    ...
    re_ = new PCRE("("+re+")", o);
    if (!re_->error().empty()) {
      KLOG(INFO) << "Cannot PCRE: " << turbo::c_encode(re);
      error_ = true;
      return;
    }
  }
...
void TestInstance::RunSearch(Engine type, std::string_view orig_text,
                             std::string_view orig_context,
  ...
  std::string_view text = orig_text;
  std::string_view context = orig_context;

  switch (type) {
    default:
      KLOG(FATAL) << "Bad RunSearch type: " << (int)type;
    ...
    case kEngineDFA1:
      ...
      if (result->matched) {
        if (!rprog_->SearchDFA(result->submatch[0], context,
                               ...
                               result->submatch,
                               &result->skipped, NULL)) {
          KLOG(ERROR) << "Reverse DFA inconsistency: "
                          << turbo::c_encode(regexp_str_)
                          << " on " << turbo::c_encode(text);
    ...
    case kEnginePCRE: {
      ...
      // catch them here. :(
      if (regexp_str_.find("\\v") != std::string_view::npos &&
          (text.find('\n') != std::string_view::npos ||
           text.find('\f') != std::string_view::npos ||
      ...
      if ((regexp_str_.find("\\s") != std::string_view::npos ||
           regexp_str_.find("\\S") != std::string_view::npos) &&
          text.find('\v') != std::string_view::npos) {
        result->skipped = true;
        break;
      }
...
// Runs a single test.
bool TestInstance::RunCase(std::string_view text, std::string_view context,
                           Prog::Anchor anchor) {
  ...
  if (correct.skipped) {
    if (regexp_ == NULL)
      return true;
    KLOG(ERROR) << "Skipped backtracking! " << turbo::c_encode(regexp_str_)
                    << " " << FormatMode(flags_);
    return false;
  }
  VKLOG(1) << "Try: regexp " << turbo::c_encode(regexp_str_)
               << " text " << turbo::c_encode(text)
               << " (" << FormatKind(kind_)
               << ", " << FormatAnchor(anchor)
               << ", " << FormatMode(flags_)
               << ")";
  ...
  for (Engine i = kEngineBacktrack+1; i < kEngineMax; i++) {
    ...
    if (ResultOkay(r, correct)) {
      if (turbo::get_flag(FLAGS_log_okay))
    ...
    if (r.matched != correct.matched) {
      if (r.matched) {
        KLOG(INFO) << "   Should not match (but does).";
      } else {
        KLOG(INFO) << "   Should match (but does not).";
        continue;
      }
    }
  ...
  if (!all_okay) {
    ...
    static int max_regexp_failures = turbo::get_flag(FLAGS_max_regexp_failures);
    if (max_regexp_failures > 0 && --max_regexp_failures == 0)
      KLOG(QFATAL) << "Too many regexp failures.";
  }
...
void TestInstance::LogMatch(const char* prefix, Engine e,
  ...
  KLOG(INFO) << prefix
    ...
    << turbo::c_encode(regexp_str_)
    << " "
    << turbo::c_encode(regexp_->ToString())
    << " text "
    << turbo::c_encode(text)
    << " ("
...
Tester::Tester(std::string_view regexp) {
  error_ = false;
  for (size_t i = 0; i < TURBO_ARRAYSIZE(kinds); i++) {
    for (size_t j = 0; j < TURBO_ARRAYSIZE(parse_modes); j++) {
...
bool Tester::TestCase(std::string_view text, std::string_view context,
                      Prog::Anchor anchor) {
  bool okay = true;
  for (size_t i = 0; i < v_.size(); i++)
...
bool Tester::TestInput(std::string_view text) {
...
bool Tester::TestInputInContext(std::string_view text,
                                std::string_view context) {
  bool okay = true;
  for (size_t i = 0; i < TURBO_ARRAYSIZE(anchors); i++)
    okay &= TestCase(text, context, anchors[i]);
  return okay;
}

bool TestRegexpOnText(std::string_view regexp,
