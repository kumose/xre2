...
int ToStringWalker::PreVisit(Regexp* re, int parent_arg, bool* stop) {
  ...
  switch (re->op()) {
    ...
    case kRegexpCapture:
      t_->append("(");
      if (re->cap() == 0)
        KLOG(DFATAL) << "kRegexpCapture cap() == 0";
      if (re->name()) {
        t_->append("?P<");
        t_->append(*re->name());
        t_->append(">");
...
int ToStringWalker::PostVisit(Regexp* re, int parent_arg, int pre_arg,
  ...
  switch (re->op()) {
    ...
    case kRegexpRepeat:
      if (re->max() == -1)
        t_->append(turbo::str_format("{%d,}", re->min()));
      else if (re->min() == re->max())
        t_->append(turbo::str_format("{%d}", re->min()));
      else
        t_->append(turbo::str_format("{%d,%d}", re->min(), re->max()));
      if (re->parse_flags() & Regexp::NonGreedy)
        t_->append("?");
      if (prec < PrecUnary)
        t_->append(")");
    ...
    case kRegexpHaveMatch:
      ...
      t_->append(turbo::str_format("(?HaveMatch:%d)", re->match_id()));
      break;
  }
...
static void AppendCCChar(std::string* t, Rune r) {
  ...
  if (r < 0x100) {
    *t += turbo::str_format("\\x%02x", static_cast<int>(r));
    return;
  }
  *t += turbo::str_format("\\x{%x}", static_cast<int>(r));
}
