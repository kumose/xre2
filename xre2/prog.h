...
namespace xre2 {
    ...
    public:
        ...
        public:
            ...
            int out1() {
                DKCHECK(opcode() == kInstAlt || opcode() == kInstAltMatch);
                return out1_;
            }

            int cap() {
                DKCHECK_EQ(opcode(), kInstCapture);
                return cap_;
            }

            int lo() {
                DKCHECK_EQ(opcode(), kInstByteRange);
                return lo_;
            }
            ...
            int match_id() {
                DKCHECK_EQ(opcode(), kInstMatch);
                return match_id_;
            }

            EmptyOp empty() {
                DKCHECK_EQ(opcode(), kInstEmptyWidth);
                return empty_;
            }

            bool greedy(Prog *p) {
                DKCHECK_EQ(opcode(), kInstAltMatch);
        ...
        private:
            ...
            union {
                ...
                //   Match ID to identify this match (for xre2::Set).
        ...
        // it is set to NULL.
        //
        // Matching text == std::string_view() is treated as any other empty
        ...
        bool UnsafeSearchBacktrack(std::string_view text, std::string_view context,
