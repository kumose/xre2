...
class BitState {
 ...
 private:
  static inline bool ShouldVisit(std::string_view text, uint64_t* visited, uint16_t id, const char* p);
...
// them multiple times.
bool BitState::ShouldVisit(std::string_view text, uint64_t* visited, uint16_t list_id, const char* p) {
...
bool BitState::TrySearch(int id0, const char* p0) {
  ...
  Loop:
    ...
    switch (ip->opcode()) {
      default:
        KLOG(DFATAL) << "Unexpected opcode: " << ip->opcode();
        return false;
      ...
      CheckAndLoop:
        ...
        DKCHECK(id == 0 || prog_->inst(id-1)->last());
...
// Bit-state search.
bool Prog::SearchBitState(std::string_view text, std::string_view context,
