...
#include <xre2/pod_array.h>
#include <xre2/prog.h>
#include <xre2/re2.h>
#include <xre2/sparse_set.h>
#include <xre2/util/strutil.h>
...
namespace xre2 {
    ...
    public:
        ...
        bool Search(std::string_view text, std::string_view context, bool anchored,
        ...
        struct State {
            ...
            template<typename H>
            friend H turbo_hash_value(H h, const State &a) {
                const turbo::span<const int> ainst(a.inst_, a.ninst_);
                return H::combine(std::move(h), a.flag_, ainst);
            }

            friend bool operator==(const State &a, const State &b) {
                const turbo::span<const int> ainst(a.inst_, a.ninst_);
                const turbo::span<const int> binst(b.inst_, b.ninst_);
        ...
        typedef turbo::flat_hash_set<State *, StateHash, StateEqual> StateSet;

    private:
        // Make it easier to swap in a scalable reader-writer mutex.
        using CacheMutex = std::shared_mutex;
...
#define MatchSep (-2)
    ...
    public:
        ...
        Workq(int n, int maxmark) : SparseSet(n + maxmark),
                                    n_(n),
                                    maxmark_(maxmark),
                                    nextmark_(n),
                                    last_was_mark_(true) {
...
#define SpecialStateMax FullMatchState
    ...
    std::string DFA::DumpState(State *state) {
        ...
        for (int i = 0; i < state->ninst_; i++) {
            if (state->inst_[i] == Mark) {
                ...
                s += turbo::str_format("%s%d", sep, state->inst_[i]);
                sep = ",";
            }
        }
        s += turbo::str_format(" flag=%#x", state->flag_);
        return s;
    }
    ...
    DFA::State *DFA::WorkqToCachedState(Workq *q, Workq *mq, uint32_t flag) {
        ...
        if (ExtraDebug)
            turbo::FPrintF(stderr, "WorkqToCachedState %s [%#x]", DumpWorkq(q), flag);
        for (Workq::iterator it = q->begin(); it != q->end(); ++it) {
            int id = *it;
            ...
            switch (ip->opcode()) {
                case kInstAltMatch:
                    ...
                    if (kind_ != Prog::kManyMatch &&
                        ...
                        (flag & kFlagMatch)) {
                        if (ExtraDebug)
                            turbo::FPrintF(stderr, " -> FullMatchState\n");
        ...
        if (n == 0 && flag == 0) {
            if (ExtraDebug)
                turbo::FPrintF(stderr, " -> DeadState\n");
            return DeadState;
        }
    ...
    void DFA::AddToQueue(Workq *q, int id, uint32_t flag) {
        ...
        Loop:
            ...
            switch (ip->opcode()) {
                default:
                    KLOG(DFATAL) << "unhandled opcode: " << ip->opcode();
                    break;
    ...
    void DFA::RunWorkqOnByte(Workq *oldq, Workq *newq,
        ...
        if (ExtraDebug)
            turbo::FPrintF(stderr, "%s on %d[%#x] -> %s [%d]\n",
    ...
    DFA::State *DFA::RunStateOnByte(State *state, int c) {
        ...
        if (state <= SpecialStateMax) {
            ...
            if (state == DeadState) {
                KLOG(DFATAL) << "DeadState in RunStateOnByte";
                return NULL;
            }
            if (state == NULL) {
                KLOG(DFATAL) << "NULL state in RunStateOnByte";
                return NULL;
            }
            KLOG(DFATAL) << "Unexpected special state in RunStateOnByte";
            return NULL;
        }
    ...
    void DFA::RWLocker::LockForWriting() TURBO_NO_THREAD_SAFETY_ANALYSIS {
        if (!writing_) {
            mu_->unlock_shared();
            mu_->lock();
            writing_ = true;
    ...
    DFA::State *DFA::StateSaver::Restore() {
        if (is_special_)
            return special_;
        std::unique_lock l(dfa_->mutex_);
        State *s = dfa_->CachedState(inst_, ninst_, flag_);
        if (s == NULL)
            KLOG(DFATAL) << "StateSaver failed to restore state.";
        return s;
    }
    ...
    inline bool DFA::InlinedSearchLoop(SearchParams *params) {
        ...
        State *s = start;
        if (ExtraDebug)
            turbo::FPrintF(stderr, "@stx: %s\n", DumpState(s));

        if (s->IsMatch()) {
            matched = true;
            lastmatch = p;
            if (ExtraDebug)
                turbo::FPrintF(stderr, "match @stx! [%s]\n", DumpState(s));
        ...
        while (p != ep) {
            ...
            if (ns == NULL) {
                ...
                if (ns == NULL) {
                    ...
                    if ((start = save_start.Restore()) == NULL ||
                        ...
                        // Restore already did KLOG(DFATAL).
                        params->failed = true;
                        return false;
                    }
                    ns = RunStateOnByteUnlocked(s, c);
                    if (ns == NULL) {
                        KLOG(DFATAL) << "RunStateOnByteUnlocked failed after ResetCache";
                        params->failed = true;
                        return false;
                    }
                }
            ...
            if (s->IsMatch()) {
                ...
                if (run_forward)
                    ...
                    lastmatch = p + 1;
                if (ExtraDebug)
                    turbo::FPrintF(stderr, "match @%d! [%s]\n", lastmatch - bp, DumpState(s));
                if (params->matches != NULL) {
                    for (int i = s->ninst_ - 1; i >= 0; i--) {
                        int id = s->inst_[i];
                        if (id == MatchSep)
        ...
        if (ns == NULL) {
            ...
            if (ns == NULL) {
                ...
                ns = RunStateOnByteUnlocked(s, lastbyte);
                if (ns == NULL) {
                    KLOG(DFATAL) << "RunStateOnByteUnlocked failed after Reset";
                    params->failed = true;
                    return false;
                }
            }
        ...
        if (s->IsMatch()) {
            ...
            lastmatch = p;
            if (ExtraDebug)
                turbo::FPrintF(stderr, "match @etx! [%s]\n", DumpState(s));
            if (params->matches != NULL) {
                for (int i = s->ninst_ - 1; i >= 0; i--) {
                    int id = s->inst_[i];
                    if (id == MatchSep)
    ...
    bool DFA::AnalyzeSearch(SearchParams *params) {
        ...
        if (BeginPtr(text) < BeginPtr(context) || EndPtr(text) > EndPtr(context)) {
            KLOG(DFATAL) << "context does not contain text";
        ...
        if (!AnalyzeSearchHelper(params, info, flags)) {
            ...
            if (!AnalyzeSearchHelper(params, info, flags)) {
                params->failed = true;
                KLOG(DFATAL) << "Failed to analyze start state.";
                return false;
            }
        }
        ...
        if (ExtraDebug)
            turbo::FPrintF(stderr, "anchored=%d fwd=%d flags=%#x state=%s can_prefix_accel=%d\n",
    ...
    bool DFA::Search(std::string_view text, std::string_view context,
        ...
        *failed = false;
        ...
        if (ExtraDebug) {
            ...
            turbo::FPrintF(stderr, "text %s anchored=%d earliest=%d fwd=%d kind %d\n",
        ...
        DKCHECK(matches == NULL || kind_ == Prog::kManyMatch);
    ...
    //
    bool Prog::SearchDFA(std::string_view text, std::string_view context,
    ...
    int DFA::BuildAllStates(const Prog::DFAStateCallback &cb) {
        ...
        turbo::flat_hash_map<State *, int> m;
        std::deque<State *> q;
    ...
    bool DFA::PossibleMatchRange(std::string *min, std::string *max, int maxlen) {
        ...
        turbo::flat_hash_map<State *, int> previously_visited_states;
