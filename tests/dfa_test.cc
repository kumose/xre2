...
#include <xre2/testing/string_generator.h>
#include <xre2/util/malloc_counter.h>

static const bool UsingMallocCounter = false;

TURBO_FLAG(int, size, 8, "log2(number of DFA nodes)");
TURBO_FLAG(int, repeat, 2, "Repetition count.");
TURBO_FLAG(int, threads, 4, "number of threads");
...
TEST(Multithreaded, BuildEntireDFA) {
  ...
  std::string s = "a";
  for (int i = 0; i < turbo::get_flag(FLAGS_size); i++)
    s += "[ab]";
  s += "b";
  ...
  for (int i = 0; i < turbo::get_flag(FLAGS_repeat); i++) {
    Prog* prog = re->CompileToProg(0);
    ASSERT_TRUE(prog != NULL);

    std::vector<std::thread> threads;
    for (int j = 0; j < turbo::get_flag(FLAGS_threads); j++)
...
TEST(SingleThreaded, SearchDFA) {
  ...
  Regexp* re = Regexp::Parse(turbo::str_format("0[01]{%d}$", n),
  ...
  int64_t peak_usage;
  {
    ...
    for (int i = 0; i < 10; i++) {
      ...
      bool failed = false;
      matched =
          prog->SearchDFA(match, std::string_view(), Prog::kUnanchored,
                          Prog::kFirstMatch, NULL, &failed, NULL);
      ASSERT_FALSE(failed);
      ASSERT_TRUE(matched);
      matched =
          prog->SearchDFA(no_match, std::string_view(), Prog::kUnanchored,
                          Prog::kFirstMatch, NULL, &failed, NULL);
      ASSERT_FALSE(failed);
...
static void DoSearch(Prog* prog, std::string_view match,
...
TEST(DFA, ReverseMatch) {
  int nfail = 0;
  for (size_t i = 0; i < TURBO_ARRAYSIZE(reverse_tests); i++) {
    ...
    bool failed = false;
    bool matched =
        prog->SearchDFA(t.text, std::string_view(), Prog::kUnanchored,
                        Prog::kFirstMatch, NULL, &failed, NULL);
    if (matched != t.match) {
      KLOG(ERROR) << t.regexp << " on " << t.text << ": want " << t.match;
      nfail++;
    }
    delete prog;
    re->Decref();
...
TEST(DFA, Callback) {
  int nfail = 0;
  for (size_t i = 0; i < TURBO_ARRAYSIZE(callback_tests); i++) {
    ...
    prog->BuildEntireDFA(Prog::kLongestMatch, [&](const int* next, bool match) {
      ...
      for (int b = 0; b < prog->bytemap_range() + 1; b++)
        dump += turbo::str_format("%d,", next[b]);
      dump.pop_back();
      dump += match ? "]]" : "]";
    });
    if (dump != t.dump) {
      KLOG(ERROR) << t.regexp << " bytemap:\n" << prog->DumpByteMap();
      KLOG(ERROR) << t.regexp << " dump:\n" << "got " << dump << "\n"
                      << "want " << t.dump;
      nfail++;
    }
    delete prog;
