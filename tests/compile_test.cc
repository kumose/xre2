...
TEST(TestRegexpCompileToProg, Simple) {
  ...
  for (size_t i = 0; i < TURBO_ARRAYSIZE(tests); i++) {
    const xre2::Test& t = tests[i];
    Regexp* re = Regexp::Parse(t.regexp, Regexp::PerlX|Regexp::Latin1, NULL);
    if (re == NULL) {
      KLOG(ERROR) << "Cannot parse: " << t.regexp;
      failed++;
      continue;
    }
    Prog* prog = re->CompileToProg(0);
    if (prog == NULL) {
      KLOG(ERROR) << "Cannot compile: " << t.regexp;
      re->Decref();
      failed++;
      continue;
    }
    ...
    if (s != t.code) {
      KLOG(ERROR) << "Incorrect compiled code for: " << t.regexp;
      KLOG(ERROR) << "Want:\n" << t.code;
      KLOG(ERROR) << "Got:\n" << s;
      failed++;
    }
...
static void DumpByteMap(std::string_view pattern, Regexp::ParseFlags flags,
                        std::string* bytemap) {
...
static void Dump(std::string_view pattern, Regexp::ParseFlags flags,
