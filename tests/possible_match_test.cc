...
TEST(PossibleMatchRange, HandWritten) {
  for (size_t i = 0; i < TURBO_ARRAYSIZE(tests); i++) {
    for (size_t j = 0; j < 2; j++) {
      ...
      std::string min, max;
      if (j == 0) {
        KLOG(INFO) << "Checking regexp=" << turbo::c_encode(t.regexp);
...
TEST(PossibleMatchRange, Failures) {
  ...
  EXPECT_FALSE(RE2("[\\s\\S]+", RE2::Latin1).
      ...
      << "min=" << turbo::c_encode(min) << ", max=" << turbo::c_encode(max);
...
void PossibleMatchTester::HandleRegexp(const std::string& regexp) {
  ...
  if(!re.PossibleMatchRange(&min, &max, 10)) {
    ...
    if(strstr(regexp.c_str(), "\\C*"))
      return;
    KLOG(QFATAL) << "PossibleMatchRange failed on: "
                     << turbo::c_encode(regexp);
  }

  strgen_.Reset();
  while (strgen_.HasNext()) {
    std::string_view s = strgen_.Next();
    tests_++;
