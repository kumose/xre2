...
static void Broke(const char *desc, const CCTest* t, CharClass* cc) {
  if (t == NULL) {
    ...
    turbo::PrintF("\n");
    turbo::PrintF("CharClass added: [%s]", desc);
    for (int k = 0; t->add[k].lo >= 0; k++)
      turbo::PrintF(" %d-%d", t->add[k].lo, t->add[k].hi);
    turbo::PrintF("\n");
    if (t->remove >= 0)
      turbo::PrintF("Removed > %d\n", t->remove);
    turbo::PrintF("\twant:");
    for (int k = 0; t->final[k].lo >= 0; k++)
      turbo::PrintF(" %d-%d", t->final[k].lo, t->final[k].hi);
    turbo::PrintF("\n");
    turbo::PrintF("\thave:");
  }

  for (typename CharClass::iterator it = cc->begin(); it != cc->end(); ++it)
    turbo::PrintF(" %d-%d", it->lo, it->hi);
  turbo::PrintF("\n");
}
...
bool CorrectCC(CharClass *cc, CCTest *t, const char *desc) {
  ...
  if (cc->size() != size) {
    Broke(desc, t, cc);
    turbo::PrintF("wrong size: want %d have %d\n", size, cc->size());
    return false;
  }
  ...
  for (int j = 0; j < 101; j++) {
    ...
    if (ShouldContain(t, j) != cc->Contains(j)) {
      Broke(desc, t, cc);
      turbo::PrintF("want contains(%d)=%d, got %d\n",
  ...
  for (int j = 0; j < 101; j++) {
    ...
    if (ShouldContain(t, j) == ncc->Contains(j)) {
      Broke(desc, t, cc);
      Broke("ncc", NULL, ncc);
      turbo::PrintF("want ncc contains(%d)!=%d, got %d\n",
    ...
    if (ncc->size() != Runemax+1 - cc->size()) {
      Broke(desc, t, cc);
      Broke("ncc", NULL, ncc);
      turbo::PrintF("ncc size should be %d is %d\n",
