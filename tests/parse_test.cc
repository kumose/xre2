...
TEST(TestParse, SimpleRegexps) {
  TestParse(tests, TURBO_ARRAYSIZE(tests), kTestFlags, "simple");
}
...
TEST(TestParse, FoldCase) {
  TestParse(foldcase_tests, TURBO_ARRAYSIZE(foldcase_tests), Regexp::FoldCase, "foldcase");
}
...
TEST(TestParse, Literal) {
  TestParse(literal_tests, TURBO_ARRAYSIZE(literal_tests), Regexp::Literal, "literal");
}
...
TEST(TestParse, MatchNL) {
  TestParse(matchnl_tests, TURBO_ARRAYSIZE(matchnl_tests), Regexp::MatchNL, "with MatchNL");
}
...
TEST(TestParse, NoMatchNL) {
  TestParse(nomatchnl_tests, TURBO_ARRAYSIZE(nomatchnl_tests), Regexp::NoParseFlags, "without MatchNL");
}
...
TEST(TestParse, Prefix) {
  TestParse(prefix_tests, TURBO_ARRAYSIZE(prefix_tests), Regexp::PerlX, "prefix");
}
...
TEST(TestParse, Nested) {
  TestParse(nested_tests, TURBO_ARRAYSIZE(nested_tests), Regexp::PerlX, "nested");
}
...
TEST(TestParse, InvalidRegexps) {
  for (size_t i = 0; i < TURBO_ARRAYSIZE(badtests); i++) {
  ...
  for (size_t i = 0; i < TURBO_ARRAYSIZE(only_posix); i++) {
  ...
  for (size_t i = 0; i < TURBO_ARRAYSIZE(only_perl); i++) {
...
TEST(TestToString, EquivalentParse) {
  for (size_t i = 0; i < TURBO_ARRAYSIZE(tests); i++) {
    RegexpStatus status;
    ...
    if (t != tests[i].regexp) {
      ...
      if (s != ss || t != tt)
        KLOG(INFO) << "ToString(" << tests[i].regexp << ") = " << t;
      EXPECT_EQ(s, ss);
      EXPECT_EQ(t, tt);
      nre->Decref();
    }
