...
bool CheckExpectedAtoms(const char* atoms[],
  ...
  if (!pass) {
    KLOG(ERROR) << "Failed " << testname;
    KLOG(ERROR) << "Expected #atoms = " << expected.size();
    for (size_t i = 0; i < expected.size(); i++)
      KLOG(ERROR) << expected[i];
    KLOG(ERROR) << "Found #atoms = " << v->atoms.size();
    for (size_t i = 0; i < v->atoms.size(); i++)
      KLOG(ERROR) << v->atoms[i];
  }
...
TEST(FilteredRE2Test, AtomTests) {
  int nfail = 0;
  for (size_t i = 0; i < TURBO_ARRAYSIZE(atom_tests); i++) {
    FilterTestVars v;
    AtomTest* t = &atom_tests[i];
    size_t nregexp, natom;
    for (nregexp = 0; nregexp < TURBO_ARRAYSIZE(t->regexps); nregexp++)
      if (t->regexps[nregexp] == NULL)
        break;
    for (natom = 0; natom < TURBO_ARRAYSIZE(t->atoms); natom++)
...
TEST(FilteredRE2Test, MatchTests) {
  ...
  atoms.push_back("yyyzzz");
  FindAtomIndices(v.atoms, atoms, &atom_ids);
  KLOG(INFO) << "S: " << atom_ids.size();
  for (size_t i = 0; i < atom_ids.size(); i++)
    KLOG(INFO) << "i: " << i << " : " << atom_ids[i];
  v.f.AllMatches(text, atom_ids, &matching_regexps);
...
TEST(FilteredRE2Test, EmptyStringInStringSetBug) {
  ...
  AddRegexpsAndCompile(regexps, TURBO_ARRAYSIZE(regexps), &v);
  EXPECT_TRUE(CheckExpectedAtoms(atoms, TURBO_ARRAYSIZE(atoms),
